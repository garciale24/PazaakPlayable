'''
Monte Carlo Tree Search with no UCB Algorithm for Pazaak game. 
...
def monte_carlo_algorithmNoUCB(pazaakGame: PazaakState) -> PazaakState:
    ...
    # Pop Selected side card if one was used 
    if returnState == None:
        returnState = gameRoot.state
        returnState.P2pop = -1

    # If a dumb move was preformed, just End Turn
    if returnState.player ==2:
        if returnState.P2setVal == 15 or returnState.P2setVal == 16 or returnState.P2setVal == 14:
            if returnState.whoWon() != 2:
                return gameRoot.state

    # If a dumb move was preformed, just End Turn
    if returnState.player ==1:
        if returnState.P1setVal == 15 or returnState.P1setVal == 16 or returnState.P1setVal == 14:
            if returnState.whoWon() != 1:
                return gameRoot.state
    return returnState
