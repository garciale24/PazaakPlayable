...
def player1SideCardSelection(selected_side_card, pazaakGame, MOUSE_X, MOUSE_Y):
    i= 0
...
def player1EndTurnStand(pazaakGame, MOUSE_X, MOUSE_Y, run2):
...
def player2Actions(pazaakGame, run2):
...
def player1Actions(pazaakGame, run2):
    selected_side_card= True
    while run2:     
...
def checkRounds(rounds_checker, rounds_flag):
...
def pickTurn(rounds_flag, rounds_checker, k, pazaakGame):
    if rounds_flag == 0:
...
def addWins(pazaakGame, rounds_checker, rounds_flag, breaky):
    P1Wins = 0
    P2Wins = 0
    winner = 0
...
def addBoardCard(pazaakGame, nextCard): 
...
def play1game(pazaakGame, k, rounds_checker, rounds_flag):
    #pygame.mixer.music.play()
    run2= True
    run= True
    breaky= False
