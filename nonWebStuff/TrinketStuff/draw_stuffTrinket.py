...
''' Background '''
BACKG = pygame.image.load(os.path.join('B.jpg'))
...
def draw_winner(player, pazaakGame):
...
def get_card(nextCard):
    i = -10
    while i < 10:
        if nextCard == i:
            plus = pygame.image.load(os.path.join('plus' + str(i) + '.png'))
...
def draw_window(pazaakGame):
    i = 0
    WIN.blit(BG, (0, 0))
    font = pygame.font.Font("times.ttf", int(32* SIZE))
