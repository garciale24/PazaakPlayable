...
class RootKids:
    def __init__(self, pazaakGame, move, parenty):
        self.move = move
...
def monte_carlo_algorithm(pazaakGame):

    # Pre-MCTS stuff 
    currSelect = []
    gameRoot = RootKids(pazaakGame, 0, None)
    kidsList = []
...
def doMoveP2(pazaakGame, move) :
    childState = pazaakGame.makeChild()
# -------------------------------------------#
    if move == -10:
...
def getKidsP2(pazaak):
    pazaakGame = pazaak.state
    adder = 0
    childs = []
...
def simulation(rent):
    tempRent = RootKids(rent.state.makeChild(), rent.move, rent.parent)
    temp = 0
...
def backpropagation(winner, child, player, multiplier):
    tempy = child
