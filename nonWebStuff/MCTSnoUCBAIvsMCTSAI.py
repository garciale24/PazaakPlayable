'''
This file contains code that allows for adversarial AI's (no UCB MCTS vs Vanialla MCTS)
'''
...
''' Function to use MCTS algorithm for player 2 '''
def player2_AI(pazaakGame: PazaakState) -> None:
    if pazaakGame.P2setVal > 10: pazaakGame = monte_carlo_algorithm(pazaakGame)
    return pazaakGame

''' Function to use no UCB MCTS algorithm for player 1 '''
def player1_AI(pazaakGame: PazaakState) -> None:
    if pazaakGame.P1setVal > 10: pazaakGame = monte_carlo_algorithmNoUCB(pazaakGame)
    return pazaakGame
